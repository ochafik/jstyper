    ...
    if ((e.keyCode || e.which) == '\t'.charCodeAt(0)) {
      ...
      if (start == end && !e.shiftKey) {
        ...
        if (preTabIndex == start) {
          textArea.value = pre +
              newSelection.substring(skippedPreTabIndex - preTabIndex) +
              text.substring(end);
        } else {
          textArea.value = text.substring(0, preTabIndex) + pre +
              text.substring(skippedPreTabIndex, start) + newSelection +
              text.substring(end);
        }
        const startOffset = pre.length + preTabIndex - skippedPreTabIndex;
        textArea.selectionStart = start + startOffset;
        textArea.selectionEnd =
            end + startOffset + newSelection.length - selection.length;
      }
